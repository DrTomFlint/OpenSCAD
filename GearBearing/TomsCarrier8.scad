...
difference(){
    ...
    union(){
        difference(){
            cylinder(r1=D/2-15,r2=D/2-16,h=Hc,$fn=200);            
            cylinder(r1=D/2-18.5,r2=D/2-17.5,h=Hc,$fn=200);            
            }
        
        // The m bushings in the carrier
        for(i=[1:m])rotate([0,0,i*360/m+phi])translate([pitchD/2*(ns+np)/nr,0,0])
                    color([1,0,0]) cylinder(h=Hc, r1=4.5, r2 = 3.5, $fn=16);
        // one more for the handle
        rotate([0,0,0.5*360/m+phi])translate([pitchD/2*(ns+np)/nr,0,0])
                color([0,0,1]) cylinder(h=Hc, r1=3.5, r2 = 4.5, $fn=16);
        }
            
    // The m holes in the bushings 
	for(i=[1:m])rotate([0,0,i*360/m+phi])translate([pitchD/2*(ns+np)/nr,0,0])
                color([1,0,0]) cylinder(h=Hc, r1=2, r2 = 2, $fn=16);
    // One more hole for the handle
	rotate([0,0,0.5*360/m+phi])translate([pitchD/2*(ns+np)/nr,0,0])
                color([0,0,1]) cylinder(h=Hc, r1=2, r2 = 2, $fn=16);
}   

...
module tunnel2(T=1.2){
  ...
  difference(){
    ...
    // hollow out
    hull(){  
      translate([x1,y1+1.5,z1])  
      sphere(r=m1-T,$fn=F1);
      translate([x2,y2+1.5,z2])
      sphere(r=m2-T,$fn=F1);
    }
    ...
    translate([ (x1+x2)/2, (y1+y2)/2+50+2, (z1+z2)/2 ])
    rotate([0,0,5])
    cube([200,95,150],center=true);
  }
    
  // ear disk
  difference(){
    translate([x1,y1+3.5,z1])  
    rotate([0,0,2.5])
    rotate([90,0,0])
    cylinder(r=15,h=T,center=true,$fn=F2);
    hull(){
      translate([x1,y1+3.5,z1])  
      rotate([0,0,2.5])
      rotate([90,0,0])
      cylinder(r=8,h=3*T,center=true,$fn=F2);
      translate([x1+20,y1+3.5,z1-7.5])  
      rotate([0,0,2.5])
      rotate([90,0,0])
      cylinder(r=11.5,h=3*T,center=true,$fn=F2);
    }
  }

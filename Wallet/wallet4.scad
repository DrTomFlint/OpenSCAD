...
module frame(tol=0.15){
  ...
  difference(){
    union(){
      ...
      translate([x0/2-20,y0/2,thick+thick/8-thick/2])
      scale([1,0.5,1])
      minkowski(){
        cylinder(r=8,h=thick/4,center=true,$fn=F1);
        sphere(r=thick/2,$fn=F1);
      }
  ...
  difference(){
    hull(){
      translate([-x0/2-2*thick,off2,z0-thick/2])
      rotate([90,0,0])
      cylinder(r=1.5*thick+tol,h=6,center=true,$fn=F1);
      translate([-x0/2-2*thick,off2,1])
      cube([3*thick+2*tol,6,2],center=true);
    }
  ...
  difference(){
    union(){
      ...
      difference(){
        ...
        translate([-x0/2+3.5*thick,0,z0/2-5*thick])
        rotate([0,35,0])
        cube([2*r0,y0+40,2*r0],center=true);        
      }
      
      // thumb latch opposite one hinge
      translate([x0/2+thick/2,y0/4,z0-thick/8+thick/2])
      scale([0.5,1,1])
      minkowski(){
        cylinder(r=8,h=thick/4,center=true,$fn=F1);
        sphere(r=thick/2,$fn=F1);
      ...
      // hinge barrel
      hull(){
        translate([-x0/2-2*thick-tol,0,z0-thick/2])
        rotate([90,0,0])
        cylinder(r=1.5*thick,h=y0-2*r0,center=true,$fn=F1);
    ...
    // cut for hinge pins
    translate([-x0/2-2*thick,off2+10,z0-thick/2])
    rotate([90,0,0])
    cylinder(r=1.2,h=24+10,center=true,$fn=F1);
    translate([-x0/2-2*thick,-off2-10,z0-thick/2])
    rotate([90,0,0])
    cylinder(r=1.2,h=24+10,center=true,$fn=F1);
...
//  difference(){
  intersection(){
    union(){
      frame();
    translate([-x0/2-2*thick,0,z0-thick/2])
      rotate([0,-angle,0])
    translate([x0/2+2*thick,0,-z0+thick/2])
      lid();
      //~ //translate([0,0,-0.1])
    ...
    cube([2*x0,2*y0+6,8*z0],center=true);
  }
}

...
module frame(tol=0.15){
  ...
  difference(){
    union(){
      ...
      // latching bump
      translate([x0/2+thick/4,0,0.6*z0])
      rotate([90,0,0])
      rotate([0,0,180/8])
  ...
  difference(){
    hull(){
      translate([-x0/2-1.5*thick-tol,off2,z0-thick])
      rotate([90,0,0])
      cylinder(r=thick,h=6,center=true,$fn=F1);
    ...
    translate([-x0/2-1.5*thick-tol,off2,z0-thick])
    rotate([90,0,0])
    cylinder(r=0.8,h=8,center=true,$fn=F1);
  }
  difference(){
    hull(){
      translate([-x0/2-1.5*thick-tol,-off2,z0-thick])
...
module lid(tol=0.15){
  ...
  difference(){
    union(){
      ...
      // hinge barrel
      hull(){
        translate([-x0/2-1.5*thick-tol,0,z0-thick])
        rotate([90,0,0])
        cylinder(r=thick,h=y0-2*r0,center=true,$fn=F1);
        translate([-x0/2-0.5*thick,0,z0])
        rotate([90,0,0])
        cylinder(r=thick,h=y0-2*r0,center=true,$fn=F1);
      }
    } // end of union
    ...
    // cuts for hinges
    translate([-x0/2-1.5*thick-tol,off2,z0-thick])
    cube([3*thick,6+2*tol,2*thick],center=true);
    translate([-x0/2-1.5*thick-tol,-off2,z0-thick])
    cube([3*thick,6+2*tol,2*thick],center=true);
    
    // cut for hinge pins
    translate([-x0/2-1.5*thick-tol,off2+10,z0-thick])
    rotate([90,0,0])

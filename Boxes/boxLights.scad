...
// boxLights.scad
//
// Add led strip to inside lid of smallest display box.
//
...
//----------------------------------------------------------------------------------
// Small box, white LEDs, Adafruit PowerBoost 1000
module framec(){
...
difference(){
  ...
  // cut for tape at the top of the box only
  for(i=[0:7]){
...
//-----------------------------------------------------------------------------------
// Cut the frame in 2 so it can print without supports.
module ringc(){
...
//-----------------------------------------------------------------------------------
// Should fit entirely in the lid of the big boxes 235 x 185 x 20
// 10 leds on white backed strip, use the slightly yellow looking leds
// the clear ones are black light
module bigsize(){
  ...
  difference(){
    linear_extrude(height=20)
    offset(r=2,$fn=F2)
    square([235-4,185-4],center=true);
    
    // main cut, bevel to allow leds to show
    translate([0,84,-1])
    rotate([0,0,180])
    linear_extrude(height=20+2,scale=[1,1.4])
    translate([-(235-16)/2,0])
    offset(r=2,$fn=F2)
    square([235-4-12,185-4-90]);
    ...
    // Adafruit charger
    translate([98,-65,-0.01])
    linear_extrude(height=8.1)
    offset(r=0.5,$fn=55)
    square([39,23],center=true);
    
    // cut for the switch wires
    translate([75,-65,-0.1])
    linear_extrude(height=7.3)
    offset(r=0.5,$fn=55)
    square([10,14],center=true);

    // clearance for the battery connector
    translate([105,-50,-0.01])
    linear_extrude(height=8.1)
    offset(r=0.5,$fn=55)
    square([30,10],center=true);

    // cuts for the battery wires
    translate([80,-45,-0.01])
    linear_extrude(height=8.1)
    offset(r=0.5,$fn=55)
    square([50,8],center=true);

    translate([40,-57,-0.1+4])
    rotate([0,0,-50])
    cube([8,44,8],center=true);
        
    // cuts for led strip
    for(i=[0:12]){
      translate([i*x0-7/2*x0-41.5,89,9]){
        rotate([0,0,180])
        tapeb(tol=0.2,extra=1);
      }
    }
    
    // topside cut or strip and wiring
    translate([0,90,9])
    cube([300,5,14],center=true);
    ...
    // text
    translate([-20,-60,11.4])
    rotate([0,0,0])
    linear_extrude(height=0.605)
    text("Flint's Finest", font = "Open Sans:style=Bold", size=15,halign="center",valign="center",spacing=1.1);
...
// Big size box, uses the Adafruit PowerBoost 1000 charger + 5v dc output
bigsize();


// First medium size box, uses the SparkFun Lipo charger + 5v dc output
//~ midsize();


// Small boxes, LED ring and battery base are independent parts so can
